...
function validateCurrentStep() {
  ...
  if (!isValid) {
    formError.textContent = 'Completa i campi obbligatori di questo step prima di continuare.';
  } else {
...
form.addEventListener('submit', (event) => {
  ...
  result.innerHTML = `
    ...
    <p><strong>Scenario:</strong> ${location}, materiali ${materials.join(', ')}, quantità livello ${quantityIndex + 1}, piano ${floor}.</p>
    <p>Ti richiamiamo al numero <strong>${formData.get('phone')}</strong> per conferma definitiva e fascia oraria disponibile.</p>
  `;
  ...
  addressInput.addEventListener('input', () => {
    ...
    timer = setTimeout(async () => {
      try {
        ...
        // fallback silenzioso: input manuale sempre disponibile
      }
    }, 220);
  });
}
...
function setupGooglePlacesIfKeyPresent() {
  ...
  window.initGoogleAddress = function initGoogleAddress() {
    if (window.google?.maps?.places) {
      ...
      document.getElementById('address-hint').textContent = 'Google Places attivo: suggerimenti ufficiali indirizzo disponibili.';
    } else {
      setupAddressAutocompleteFallback();
    }
  };
